...
class Metallic : public Material {
  public:
    __host__ __device__ Metallic(const Vec3 &a, float f) : albedo(a) {
        if (f < 1)
            fuzz = f;
        else
            fuzz = 1;
    ...
    __device__ bool scatter(const Ray &r_in, const CollisionData &cd, Vec3 &attenuation, Ray &scattered,
  ...
  private:
    ...
    __device__ Vec3 randomNormalSphereGPU(curandState *local_rand_state) const {
        ...
        while (p.squared_length() >= 1.0f) {
            p = 2.0f * Vec3(curand_uniform(local_rand_state), curand_uniform(local_rand_state),
    ...
    __device__ Vec3 reflectGPU(const Vec3 &v, const Vec3 &n) const { return v - 2 * dot(v, n) * n; }
};
