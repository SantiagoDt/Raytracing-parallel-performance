...
class SceneGPU {
  public:
    ...
    __device__ SceneGPU(Object **list, size_t nobjects, size_t tmax, int depth = 50)
        : ol(list), capacity(nobjects), size(0), sky(), inf() {}

    __device__ void setList(Object **list, int numobjets) {
        ol = list;
        capacity = numobjets;
    }
    __device__ void add(Object *o) {
        ol[size] = o;
        size++;
    }
    ...
    __device__ Vec3 getSceneColor(const Ray &r, curandState *local_rand_state) {
        ...
        for (int i = 0; i < 50; i++) {
            ...
            // initially tmax = std::numeric_limits<float>::max()
