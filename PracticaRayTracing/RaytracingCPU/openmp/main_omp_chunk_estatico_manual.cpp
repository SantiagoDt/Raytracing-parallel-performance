...
int main() {
	...
	rayTracingCPU_OpenMP_Blocks_Manual(data, w, h, ns, patch_x_start, patch_y_start, patch_x_end, patch_y_end);
	
	t1 = omp_get_wtime();
	double elapsed = t1 - t0;
	writeBMP("output/imgCPU_static_manual.bmp", data, patch_x_size, patch_y_size);
	printf("Tiempo de computo: %.3f segundos\n", elapsed);
	printf("Imagen guardada como: output/imgCPU_static_manual.bmp\n");
